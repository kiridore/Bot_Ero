...
global.location = { hostname: "profile.littlero.com" };
global.document = {
  createElement(tag) {
...
checks.push(["含主页入口", inserted && inserted.innerHTML.includes("nav.littlero.com")]);
checks.push(["当前域高亮", inserted && inserted.innerHTML.includes('class="active"') && inserted.innerHTML.includes("profile.littlero.com")]);
checks.push(["含全部子域", inserted && ["gallery", "trpg", "guestbook", "alarms", "activities"].every((h) => inserted.innerHTML.includes(h + ".littlero.com"))]);
