...
| 31 | `activity` | `activity/` | 自定义 match: 群聊 `/活动` + 私聊 `/提交` | 群活动：接龙（每人限时、机器人接力转发）、匹配下家（圆桌单环，开始通知下家、作品玩家自提）与征集（各自在截止前提交一次，无链环），结束自动归档；开始报名/正式开始/结束向时间线发事件（source `activity`，actor=小埃同学） |
