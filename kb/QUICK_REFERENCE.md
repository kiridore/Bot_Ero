...
| `/活动 创建 征集 <标题> [描述] 截止 <时间>` | 发布征集（必填 `截止 <时间>`，可选 `报名截止`；成员各自在截止前提交一次作品，可更新） | 任何人 |
| `/活动 加入 [编号]` / `/活动 退出 [编号]` | 报名 / 退出活动（单群可并行多个活动；省略编号且唯一候选时直选，多个需带编号，编号见 `/活动 状态`） | 任何人 |
| `/活动 开始 [编号]` | 开始活动 | 创建人/超管 |
| `/活动 状态 [编号]` | 查看活动进度（无编号：唯一活动显详情，多个活动列总览） | 任何人 |
| `/活动 结束 [编号]` | 结束活动 | 创建人/超管 |
