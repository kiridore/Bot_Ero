...
class LLMChatPlugin(Plugin):
    ...
    def _persist_message(self, msg: str, user_name: Optional[str] = None, user_id: Optional[int] = None):
        ...
        with self._memory_lock:
            ...
            if (
                ...
                snapshot = list(last_300[-CHAT_HISTORY_LIMIT:])
    ...
    @staticmethod
    def _memory_extract_prompt_300(raw_dialogue: str):
        return (
            "# Role: 记忆沉淀处理器\n"
            "## Task\n"
            "阅读 300 条原始对话，生成以下两部分后端记忆：\n\n"
            "### Part 1: 群组事件摘要 (Group History)\n"
            "- 提取发生的核心事件（如：谁发起了挑战、群内气氛的变化、重要的梗）。\n"
            "- 语气客观简练。\n\n"
    ...
    def _process_summarize_job(cls, job: dict):
        ...
        user_prompt = cls._memory_extract_prompt_300(raw_dialogue)
        try:
        ...
        with cls._memory_lock:
            ...
            if group_history:
                mem["group_summary"] = group_history
