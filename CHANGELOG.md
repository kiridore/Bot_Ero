...
- **优化**：活动详情页参与人员表改为固定单行布局——昵称列弹性伸展（超长省略号，悬停可看全名），提交时间/预计截止恒定钉在行右缘不再受折行影响，匹配活动下家信息同样右对齐

## [1.49.0]

- **新增**：活动详情页参与人员表补时间信息——已完成者显示提交时间（进行中不再向他人隐藏时间戳，作品内容/图片照旧隐藏）；未提交的接龙成员显示预计截止（当前棒 = 实际接棒时刻 + 每人限时，队尾依次链式顺延，与活动全局截止取较早者），报名期与匹配/征集不显示

## [1.48.0]

- **新增赞赏页**（`/donate`，侧边栏「赞赏支持」）：展示站长手动上传的收款码图片（上传到服务器 `server_data/donate/` 目录即自动展示，支持 jpg/png/webp/gif，无文件名约定）；登录后可见，点击图片新标签页打开原图方便手机扫码

## [1.47.5]

- **修复**：打卡图文件名含 URL 不安全字符（如 `S$Y6V]1L]HO]]R%1RP8ML.png`，QQ 图片原始名）时，时间线与打卡图库图片全部无法显示——未编码的 `%1R` 构成非法 URL 转义，Caddy 反代直接 400；图库/个人中心的图片与缩略图 URL 现对文件名 percent-encode，时间线存量事件在读侧幂等归一，无需迁移

## [1.47.4]

- **修复**：接龙活动提交带图片的作品后，下一位应收到的「接力作品」私聊转发发送失败——图片消息段引用的是裸文件名，LLOneBot 无法解析（协议仅接受 `file://`、http(s) 或 `get_image` 的 file_id），导致整条转发消息发不出去；现改用 OneBot 侧可见的 `file://` 路径（python/onebot 双路径换根），纯文字提交不受影响

## [1.47.3]

- **修复**：议事厅投票到期自动关闭产生的时间线事件 actor 曾传 QQ 0，无法解析昵称/头像、时间线上显示「0」；现与周报/活动事件一致以 bot 本体为 actor（历史遗留的旧事件仍显示 0，仅影响存量）

## [1.47.2]

- **修复**：议事厅 tag 管理页新 tag 名输入框按回车会误触创建提交，现只能点「创建」按钮提交（与发帖页同款回车防护）。评论框为多行 textarea，回车本就是换行不提交，无需改动

## [1.47.1]

- **修复**：议事厅新建/编辑投票帖时，在投票问题、投票选项等输入框内按回车会误触发表单提交、直接把帖子发出去——回车误触防护改为挂在表单上的事件委托，覆盖所有单行输入（含动态创建的投票输入框），富文本正文换行与「发布」按钮回车提交不受影响
...
- **单群多活动并行**：解除「一个群同时只能有一个 open/running 活动」限制（bot 与 web 创建均不再互斥）；`/活动 加入｜退出｜开始｜状态｜结束` 支持可选 `[编号]`（编号=全局活动 id），省略时唯一候选自动选中、多个候选列出编号要求指定；`/活动 状态` 无编号且多活动时输出总览列表；`/提交 [活动id]` 既有定向语义不变
...
- **新活动类型「征集」**：`/活动 创建 征集 <标题> [描述] 截止 <时间>`——成员报名后各自在截止前提交一次作品（可更新，web 增量图片语义直接继承），无接龙流转、无匹配配对；报名/开始/退出/到点收尾/归档全复用现有骨架，web 创建页新增「征集」类型（截止必填）；归档作品标题「作品」

## [1.41.0]

- **活动详情支持富文本**：活动描述（创建页/管理页）改用共享 Tiptap 富文本编辑器（议事厅同款：esm.sh 动态加载，12s 超时降级），存储为 TipTap JSON 文档（非 HTML，天然免 XSS）；详情页与列表页按白名单节点类型渲染（历史纯文本描述向前兼容）；bot 创建路径纯文本自动转 JSON、QQ 播报与归档侧转回纯文本；编辑器/渲染器抽取为共享模块 `core/web/static/richtext.js|css` 与 `core/tiptap.py`（论坛正文/评论改用同一实现，论坛独立样式删减）
...
- **网页打卡时间线事件丢失**：打卡路由为 async，但其内的同步时间线发送会自 POST 本进程——事件循环被阻塞无法服务该自回环请求，双方互相等待双双超时，事件静默丢失（打卡响应同时卡顿约 10 秒）。阻塞工作改经线程池执行，事件正常落库且响应回到毫秒级

## [1.37.1] - 2026-09-02

### 变更

- **插件启用逻辑改为严格白名单**：取消全新部署时对默认群的全量播种——任何插件（含新部署、新装机器）初始均为禁用，需经 `/插件` 指令或 bot 监控面板按群/私聊逐个启用；系统插件不受影响
...
- **周报仙人彩头版误报无人中奖**：头版头条此前只把 ≥3A 大奖当「有人中奖」，无人命中大奖时即使有三等奖（2A）中奖者也显示「无人中奖」——现分三态：大奖落定 / 有三等奖中奖（显示中奖注数、名单与总奖池）/ 真无人中奖
- **周报仙人彩奖池数字**：奖池此前只显示本期投注额（不含滚存），与开奖公告不一致——开奖时把总池（本期投注+当时滚存）落库，周报读真实总池（历史期旧数据回退投注额）

## [1.35.1] - 2026-09-02

### 修复

- **周报翻期按钮方向**：报纸页「上一期/下一期」跳转方向颠倒（上一期跳到了更新一期、下一期跳到更旧一期），已纠正——上一期=过去一周、下一期=未来一周，最新/最旧一期的按钮禁用边界同步修正
...
- **详情隐私**：活动进行中，详情 API 不再下发其他成员的作品内容（本人与已结束归档不受影响）；进行中活动图片仅限本人/创建人访问
...
- **日程页详情弹窗排版**：闹钟详情弹窗内容与按钮区补内边距（不再贴边框）；当日列表悬浮窗末行分隔线与底部间距修正
