...
### 删除 `GET /api/me/alarms`（旧列表）

新页日历 + 悬浮窗已覆盖全部信息，无其他消费者；`list_alarms`/`_format_alarm_row` 一并删（死代码不留）。`DELETE /api/me/alarms/{id}` 原样保留（已天然支持群闹钟取消）。
...
- 提交成功 → 刷新日历 + 重置表单；服务端 400（如提前量不足 5 分钟）→ 表单上方红字提示
- 编辑模式额外带「放弃编辑」按钮
