...
| `GET` | `/api/tools/icon` | 可选 | 卡片图标（`?domain=`，仅已收录链接的域名；服务端解析 `https://<domain>/favicon.ico` → 失败抓首页 `<link rel="icon">`，结果入库缓存，无图标 404 负缓存 7 天；内网/回环地址拒绝防 SSRF；上游超时 4-5s、体积 ≤512KB、重定向 ≤3 跳） |
...
| `tools` | `/tools` 工具箱（链接卡片网格 + tag 云（全部 tag 及使用数量，点击筛选）+ tag 徽标/筛选 + 双维度排序 + 点击统计 + 关键字搜索 + 卡片/列表双视图，卡片 icon 服务端解析（favicon.ico → 首页 link rel=icon，入库缓存），头部操作/删除为图标按钮（自托管 lucide SVG，眼睛图标示点击数），添加需登录，登录用户可编辑/删除自己提交的链接） |
